 ...
 *				This prevents discontinuities around old 50% sample point.
   ...
   1.1 -> 1.2 : Added velocity control in 1 kHz interrupt
			    incoming data from data1 is no longer i_setpoint, but setpoint. Velocity control can be found in function VelocityControl
...
static void VelocityControl(void)
{
	...
	if(intcnt++ >= measure_time)
	{
		...
		// If difference is exactly on reset value (65536->0), calculate correct difference
		if(iDifference > 60000)
		{
			iPreviousValue = 65536 - iPreviousValue;
