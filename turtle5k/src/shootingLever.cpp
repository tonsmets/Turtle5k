...
#include "hardware/headers/ShootingLever.h"
...
void frameCallback(const turtle5k::ShootingLever &aMessage)
{
	...
	shootingLever.setAngle(aMessage.pShootingAngle);
...
void frameCallbackShoot(const std_msgs::Int32 &aMessage)
{
	...
	shootingLever.shoot((int)aMessage.data);
...
int main(int argc, char** argv) {
	...
	while(ros::ok()) {
		...
		double angle = shootingLever.getAngle();
